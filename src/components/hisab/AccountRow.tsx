...
export function AccountRow({ entity, balance }: { entity: Entity; balance: EntityBalance }) {
  ...
  const merchant = findMerchant(entity.name);
  const relationship = entity.relationship || merchant?.relationship;
  const avatar = entity.avatar || merchant?.logo;
  ...
  return (
    ...
    >
      <InitialsBadge name={entity.name} avatarUrl={avatar} size={46} bg={style.bg} fg={style.fg} />
